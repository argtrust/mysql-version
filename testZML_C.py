...
def clearSessionToReargue(connection, cursor, sessionID, timestep):
    try:
        ...
        connection.rollback()
    try:
        cursor.execute("delete from questions where isAttack = 1 and sessionID = %s and timestep = %s;",
                       (sessionID, timestep))
        connection.commit()
    except:
        connection.rollback()
    try:
        cursor.execute("update questions set isSupported = 0, isProcessed = 0 where sessionID = %s and timestep =  %s;",
                       (sessionID, timestep))
        connection.commit()
    except:
        connection.rollback()
    try:
        cursor.execute("delete from argument_attacks_argument where sessionID = %s and timestep = %s;",
                       (sessionID, timestep))
        connection.commit()
    except:
        connection.rollback()
    try:
        cursor.execute("delete from question_attacks_arguments where sessionID = %s and timestep = %s;",
                       (sessionID, timestep))
        connection.commit()
    except:
        connection.rollback()
    ...
    try:
        cursor.execute(
            "delete paa.* from parent_argument pa inner join parent_argument_has_argument paa where pa.parentArgumentID = paa.parentArgumentID and sessionID = %s and timestep = %s;",
            (sessionID, timestep))
        connection.commit()
    except:
        connection.rollback()
    try:
        ...
        connection.rollback()
    try:
        cursor.execute("delete from agent_has_beliefs where isInferred = 1 and sessionID = %s and timestep = %s;",
                       (sessionID, timestep))
        connection.commit()
    except:
        connection.rollback()
    try:
        cursor.execute("delete from agent_trust where isInferred = 1 and sessionID = %s and timestep = %s;",
                       (sessionID, timestep))
        connection.commit()
    except:
        connection.rollback()
...
def getPredicateHasConstantID(connection, cursor, PredicateText):
    ...
    if predicateID > 0 and constantID > 0:
        cursor.execute(
            "select predicateConstantID from predicate_has_constant where predicateID =  %s and constantID = %s;",
            (predicateID, constantID))
        if cursor.rowcount == 0:
            try:
                cursor.execute("Insert INTO predicate_has_constant (predicateID,constantID) values(%s,%s);",
                               (predicateID, constantID))
                connection.commit()
...
def getBeliefID(connection, cursor, isNegated, conclusionID, isRule):
    cursor.execute("select beliefID from beliefs where conclusionID =  %s and isNegated = %s and isRule = %s;",
                   (conclusionID, isNegated, isRule))
    #Insert record for every rule
    if isRule == 1 or cursor.rowcount == 0:
        try:
            cursor.execute("Insert INTO beliefs (conclusionID,isNegated,isRule) values(%s,%s,%s);",
                           (conclusionID, isNegated, isRule))
            connection.commit()
            return cursor.lastrowid
        except:
...
def getAgentHasBeliefsID(con, cursor, agentID, beliefID, level, isInferred, sessionID, timestep):
    try:
        cursor.execute(
            "Insert INTO agent_has_beliefs (agentID,beliefID,level,isInferred,sessionID,timestep) values(%s,%s,%s,%s,%s,%s);",
            (agentID, beliefID, level, isInferred, sessionID, timestep))
        con.commit()
        return cursor.lastrowid
    except:
        con.rollback()
...
def getAgentBeliefsText(con, cursor, agentID, beliefID, sessionID, timestep, scenariotext):
    try:
        cursor.execute(
            "Insert INTO agent_belief_text (agentID,beliefID,sessionID,timestep,scenario_text) values(%s,%s,%s,%s,%s);",
            (agentID, beliefID, sessionID, timestep, scenariotext))
        con.commit()
        return cursor.lastrowid
    except:
        con.rollback()
...
def getPremiseID(con, cursor, beliefID, isNegated, premiseID):
    try:
        cursor.execute("Insert INTO belief_has_premises (beliefID,premiseID,isNegated) values(%s,%s,%s);",
                       (beliefID, premiseID, isNegated))
        con.commit()
        return cursor.lastrowid
    except:
        con.rollback()
...
def getQuestionID(connection, cursor, agentID, isNegated, conclusionID, isAttack, sessionID, timestep):
    cursor.execute(
        "select questionID from questions where agentID =  %s and conclusionID = %s and isNegated=%s and sessionID=%s and timestep=%s;",
        (agentID, conclusionID, isNegated, sessionID, timestep))
    if cursor.rowcount == 0:
        try:
            cursor.execute(
                "Insert INTO questions (agentID,conclusionID,isAttack,isProcessed,isSupported,isNegated,sessionID,timestep) values(%s,%s,%s,0,0,%s,%s,%s);",
                (agentID, conclusionID, isAttack, isNegated, sessionID, timestep))
            connection.commit()
...
def getQuestionHasAttacksID(connection, cursor, questionID, argumentID, sessionID, timestep):
    cursor.execute(
        "select questionArgumentID from question_attacks_arguments where questionID =  %s and argumentID = %s and sessionID=%s and timestep=%s;",
        (questionID, argumentID, sessionID, timestep))
    if cursor.rowcount == 0:
        try:
            cursor.execute(
                "Insert INTO question_attacks_arguments (questionID,argumentID,sessionID,timestep) values(%s,%s,%s,%s);",
                (questionID, argumentID, sessionID, timestep))
            connection.commit()
            return cursor.lastrowid
        except mdb.Error, e:
...
def inferTrusts(connection, cursor, agentID, startingAgentID, sessionID, timestep):
    getcontext().prec = 2
    cursor.execute(
        "select trustingAgent,trustedAgent,level,isInferred from agent_trust where trustingAgent = %s and NOT trustedAgent = %s and sessionID=%s and timestep=%s;",
        (agentID, startingAgentID, sessionID, timestep))
    for trust_rel in cursor.fetchall():
        inferTrusts(connection, cursor, trust_rel[1], startingAgentID, sessionID, timestep)
        cursor.execute(
            "select trustingAgent,trustedAgent,level,isInferred,trustID from agent_trust where trustingAgent = %s and NOT trustedAgent in (%s,%s) and sessionID = %s and timestep = %s;",
            (trust_rel[1], agentID, startingAgentID, sessionID, timestep))
        for second_trust_rel in cursor.fetchall():
            inferredLevel = trust_rel[2] * second_trust_rel[2]
            try:
                cursor.execute(
                    "insert into agent_trust (trustingAgent,trustedAgent,level,isInferred,sessionID,timestep) values(%s,%s,%s,%s,%s,%s);",
                    (agentID, second_trust_rel[1], inferredLevel, 1, sessionID, timestep))
                trustID = cursor.lastrowid
                connection.commit()
                cursor.execute(
                    "insert into inferred_trust (inferredTrustID,appliedTrustID,sessionID,timestep) values(%s,%s,%s,%s);",
                    (trustID, second_trust_rel[3], sessionID, timestep))
                connection.commit()
            except:
                connection.rollback()


def processTrust(connection, cursor, sessionID, timestep, agentID):
    sql = """   select trustingAgent as ta
            from questions q
            inner join agent_trust at1 on q.sessionID = at1.sessionID and q.timestep = at1.timestep
            where q.agentID != at1.trustingAgent and at1.sessionID = %s and at1.timestep = %s
            union
            select trustedAgent as ta
            from questions q
            inner join agent_trust at1 on q.sessionID = at1.sessionID and q.timestep = at1.timestep
            where q.agentID != at1.trustedAgent and at1.sessionID = %s and at1.timestep = %s
            ;
    """
    ...
    #		inferTrusts(con,cursor,agent[0],agent[0],sessionID,timestep)
        #print "calcing trust for %s to %s" % (agentID,agent[0])
        recursiveTrust(connection, cursor, agentID, agent[0], sessionID, timestep)


def setBFSState(connection, cursor, agentID, sessionID, timestep, stateVal):
    cursor.execute("select * from agent_trust_bfs where agentID = %s and sessionID = %s and timestep = %s;",
                   (agentID, sessionID, timestep))
    if cursor.rowcount == 0:
        try:
            cursor.execute("insert into agent_trust_bfs (agentID,sessionID,timestep,bfs_state) values(%s,%s,%s,%s);",
                           (agentID, sessionID, timestep, stateVal))
            connection.commit()
        except:
            connection.rollback()
    else:
        #print "update agent_trust_bfs set bfs_state = %s where agentID = %s and sessionID = %s and timestep = %s;" % (stateVal, agentID, sessionID, timestep)
        try:
            cursor.execute(
                "update agent_trust_bfs set bfs_state = %s where agentID = %s and sessionID = %s and timestep = %s;",
                (stateVal, agentID, sessionID, timestep))
            connection.commit()
        except:
            print "except s"
            connection.rollback()

# Select agent to agent trust - if it exists return value
# If it doesn't exist
# BFS - My trust to neighbors
# implemented a DFS not BfS - not sure implications
def recursiveTrust(connection, cursor, agentFromID, agentToID, sessionID, timestep):
    getcontext().prec = 2
    #print "calcing trust in recursive for %s to %s" % (agentFromID,agentToID)
    setBFSState(connection, cursor, agentFromID, sessionID, timestep, 1)
    cursor.execute(
        "select trustingAgent,trustedAgent,level,isInferred from agent_trust where trustingAgent = %s and trustedAgent = %s and sessionID = %s and timestep = %s;",
        (agentFromID, agentToID, sessionID, timestep))
    if cursor.rowcount == 0:
        #Recurse through to get trust value
        cursor.execute(
            "select trustingAgent,trustedAgent,level,isInferred from agent_trust at1 left outer join agent_trust_bfs ab on at1.trustedAgent = ab.agentID and at1.sessionID = ab.sessionID and at1.timestep=ab.timestep where trustingAgent = %s and at1.sessionID = %s and at1.timestep = %s  and (ab.bfs_state is null or ab.bfs_state = 2);",
            (agentFromID, sessionID, timestep))
        denominator = 0
        numerator = 0
        tidalVal = 0
        for neighbor in cursor.fetchall():
            #print neighbor[1]
            #print agentToID
#            setBFSState(connection, cursor, neighbor[1], sessionID, timestep, 1)
            trustVal = recursiveTrust(connection, cursor, neighbor[1], agentToID, sessionID, timestep)
            if trustVal > 0:
                numerator = neighbor[2] * trustVal + numerator
                denominator = denominator + neighbor[2]
#                numerator =  trustVal + numerator
#                denominator = denominator + neighbor[2]
            #print numerator
        #print denominator
        if numerator > 0 and denominator > 0:
            tidalVal = numerator / denominator
            try:
                cursor.execute(
                    "insert into agent_trust (trustingAgent,trustedAgent,level,isInferred,sessionID,timestep) values(%s,%s,%s,%s,%s,%s);",
                    (agentFromID, agentToID, tidalVal, 1, sessionID, timestep))
                trustID = cursor.lastrowid
                connection.commit()
                cursor.execute(
                    "insert into inferred_trust (inferredTrustID,appliedTrustID,sessionID,timestep) values(%s,%s,%s,%s);",
                    (trustID, -1, sessionID, timestep))
                connection.commit()
            except:
                connection.rollback()
        setBFSState(connection, cursor, agentFromID, sessionID, timestep, 2)
        return tidalVal
    else:
        trustVal = 0
        for aid in cursor.fetchall():
...
def inferBeliefs(connection, cursor, agentID, sessionID, timestep):
    getcontext().prec = 2
    sql = """insert into agent_has_beliefs
            (agentID, beliefID, level, isInferred, sessionID, timestep)
            select %s, beliefID, sum(theMult)/sum(beliefLevel) as newLevel, 1, %s, %s
            from (
            select agentBeliefID, agentID, beliefID, ab.level as beliefLevel, at.level, at.level * ab.level as theMult
            from agent_has_beliefs ab
            inner join agent_trust at on
                trustedAgent = agentID and
                ab.sessionID = at.sessionID and
                ab.timestep = at.timestep
            where trustingAgent= %s and ab.sessionID = %s and ab.timestep = %s and ab.isInferred = 0
            and ab.beliefID not in (select beliefID from agent_has_beliefs where sessionID = %s and  timestep = %s and agentID = %s)
            ) x
            group by beliefID;"""

#    cursor.execute(
#        "select agentBeliefID, agentID, beliefID, ab.level, at.level from agent_has_beliefs ab inner join agent_trust at on trustedAgent = agentID and ab.sessionID = at.sessionID and ab.timestep = at.timestep where trustingAgent= %s and ab.sessionID = %s and ab.timestep = %s;",
#        (agentID, sessionID, timestep))
    try:
        cursor.execute(
            sql,
            (agentID, sessionID, timestep, agentID, sessionID, timestep, sessionID, timestep, agentID))
        connection.commit()
    except mdb.Error, e:
        print "Error %d: %s" % (e.args[0], e.args[1])
        connection.rollback()
...
"""    for belief in cursor.fetchall():
        ...
        try:
            cursor.execute(
                "insert into agent_has_beliefs (agentID,beliefID,level,isInferred,sessionID,timestep) values(%s,%s,%s,%s,%s,%s);",
                (agentID, belief[2], inferredLevel, 1, sessionID, timestep))
            agentBeliefID = cursor.lastrowid
            connection.commit()
            cursor.execute(
                "insert into inferred_beliefs (inferredBeliefID,appliedBeliefID,sessionID,timestep) values(%s,%s,%s,%s);",
                (agentBeliefID, belief[0], sessionID, timestep))
            connection.commit()
        except mdb.Error, e:
            print "Error %d: %s" % (e.args[0], e.args[1])
            connection.rollback() """
...
def argue(connection, cursor, conclusionID, isNegated, agentID, questionID, supportsArgumentID, sessionID, timestep):
    ...
    for conclusion in cursor.fetchall():
        try:
            cursor.execute(
                "insert into arguments (beliefID,questionID,supportsArgumentID,isLeaf,isSupported,sessionID,timestep) values(%s,%s,%s,%s,%s,%s,%s);",
                (conclusion[0], questionID, supportsArgumentID, 0, 0, sessionID, timestep))
            argumentID = cursor.lastrowid
            connection.commit()
            #Insert counter arguments

            if conclusion[1] == 1:
                cursor.execute("select premiseID, isNegated from belief_has_premises where beliefID = %s;",
                               (conclusion[0]))
                for premise in cursor.fetchall():
                    argue(connection, cursor, premise[0], premise[1], agentID, questionID, argumentID, sessionID,
                          timestep)
...
def createParentArgs(connection, cursor, sessionID, timestep):
    ...
    for argument in cursor.fetchall():
        ...
        for l in listOfArgs:
            cursor.execute(
                "insert into parent_argument (argumentID, questionID,sessionID,timestep) values (%s,%s,%s,%s)",
                (argument[0], argument[1], sessionID, timestep))
            con.commit()
            parentID = cursor.lastrowid
            for a in l:
                cursor.execute("insert into parent_argument_has_argument (parentArgumentID, argumentID) values (%s,%s)",
                               (parentID, a))
                con.commit()
...
def loadNewFile(inputfile, con, cursor, sessionID, timestep):
    ...
    #		''.join(node_text(n) for n in el.xpath('/node()'))
        try:
            cursor.execute("Insert INTO scenarios (sessionID,timestep,scenario_text) values(%s,%s,%s);",
                           (sessionID, timestep, et.tostring(el)))
            con.commit()
        except:
            con.rollback()
    ...
    for el in tree.findall('trustnet/trust'):
        ...
        try:
            cursor.execute(
                "Insert INTO agent_trust (trustingAgent,trustedAgent,level,isInferred,sessionID,timestep) values(%s,%s,%s,%s,%s,%s);",
                (trusterID, trustedID, level, 0, sessionID, timestep))
            con.commit()
        except:
            con.rollback()
        try:
            cursor.execute(
                "Insert INTO agent_trust_text (trustingAgent,trustedAgent,sessionID,timestep,scenario_text) values(%s,%s,%s,%s,%s);",
                (trusterID, trustedID, sessionID, timestep, textAboutTrust))
            con.commit()
        except:
            con.rollback()
    ...
    for el in tree.findall('query'):
        ...
        questionID = getQuestionID(con, cursor, agentID, predicateHasConstant[0], predicateHasConstant[1], isAttack,
                                   sessionID, timestep)

    cursor.execute("select agentID from questions where sessionID = %s and timestep = %s;", (sessionID, timestep))
    processTrust(con, cursor, sessionID, timestep, agentID)
    #print "processing trust for %s" % (agentID)
...
def reloadArgument(con, cursor, sessionID, timestep):
    cursor.execute(
        "select questionID, agentID,conclusionID,isNegated,isAttack from questions where isAttack = 0 and sessionID=%s and timestep=%s;",
        (sessionID, timestep))
    if cursor.rowcount == 0:
        try:
        ...
        for aid in cursor.fetchall():
            ...
            cursor.execute("select agentID from questions where sessionID = %s and timestep = %s;",
                           (sessionID, timestep))
            for agent in cursor.fetchall():
                #print "processing trust for agent %s" % (agent[0])
                processTrust(con, cursor, sessionID, timestep, agent[0])
            #				inferTrusts(con,cursor,agent[0],agent[0],sessionID,timestep)
...
try:
    ...
    if (args.inputfile):
        ...
        cursor.execute("insert into user_session(sessionID,date,current_timestep,userid) values (%s,NOW(),%s,%s);",
                       (sessionID, timestep, args.userid))
        con.commit()
        #		resetDB(con,cursor)
