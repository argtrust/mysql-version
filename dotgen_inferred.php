...
$sql="select distinct b.beliefID, CASE 
 ...
 if ($result) {
  while ($row = mysqli_fetch_array($result)) {
  	...
  	printf ("rule%s [label=\"%s :- %s:%s\", shape=box3d,  fontsize=50, fillcolor=lightblue, href=\"javascript:void(0)\", onclick=\"get_id('\L', '\N')\"];\n",$row[0],$row[1],$premise,$row[2]);
  	printf ("inference%s [label=\"%s\", shape=box, fillcolor=lightcyan, fontsize=50, href=\"javascript:void(0)\", onclick=\"get_id('\L', '\N')\"];\n",$row[0],$row[1]);
	printf ("rule%s -> inference%s [color=darkgreen,  fontsize=50, href=\"javascript:void(0)\", onclick=\"get_id('\L', '\N')\"];\n",$row[0],$row[0]);
  }
}
 ...
 if ($result) {
  while ($row = mysqli_fetch_array($result)) {
	...
	printf ("rule%s -> inference%s [color=darkgreen,  fontsize=50, href=\"javascript:void(0)\", onclick=\"get_id('\L', '\N')\"];\n",$row[0],$row[0]);
  	...
  	if($row[3] == "IN" && $row[4] == 1){
		printf ("inference%s [label=\"%s:%s : %s\", shape=box,  fontsize=50, fillcolor=palegreen, href=\"javascript:void(0)\", onclick=\"get_id('\L', '\N')\"];\n",$row[0],$row[1],$row[2],$row[3]);
  	}else if($row[3] == "OUT" && $row[4] == 1){
		printf ("inference%s [label=\"%s:%s : %s\", fontsize=50, style=\"filled\", fillcolor=pink, shape=box, href=\"javascript:void(0)\", onclick=\"get_id('\L', '\N')\"];\n",$row[0],$row[1],$row[2],$row[3]);
  	}else if($row[3] == "UNDEC" && $row[4] == 1){
		printf ("inference%s [label=\"%s:%s : %s\", fontsize=50, shape=box, fillcolor=grey, href=\"javascript:void(0)\", onclick=\"get_id('\L', '\N')\"];\n",$row[0],$row[1],$row[2],$row[3]);
  	}else{
  		$statuses = "";
		$count = 0;
		$sql = "select status 
		...
		printf ("inference%s [label=\"%s:%s : %s\", style=\"dotted, filled\" shape=box,  fontsize=50, fillcolor=lemonchiffon, href=\"javascript:void(0)\", onclick=\"get_id('\L', '\N')\"];\n",$row[0],$row[1],$row[2],$statuses);
...
//Fact premise nodes that aren't conclusions
$sql = "select distinct b2.beliefID, CASE 
        WHEN b.isNegated=1 THEN concat('NOT(',p.name,'(',c.name,'))') 
		...
		inner join predicates p on p.predicateID = pc.predicateID
		inner join constants c on pc.constantID = c.constantID 
	    inner join beliefs b2 on b.premiseID = b2.conclusionID and b.isNegated = b2.isNegated
	    inner join agent_has_beliefs ab on b2.beliefID = ab.beliefID
	    inner join questions q on q.agentID = ab.agentID and q.sessionID = ab.sessionID and q.timestep = ab.timestep
			where b.beliefID = ".$ruleID." and ab.sessionID = '".$sessionID."' and ab.timestep = ".$timestep."
					and b2.isRule = 0
					and b2.beliefID NOT IN (select distinct b.beliefID 
									from arguments a 
									inner join beliefs b on b.beliefID = a.beliefID 
									inner join parent_argument pa on pa.argumentID = a.argumentID 
												and pa.sessionID = a.sessionID 
 ...
 if ($result) {
  while ($row = mysqli_fetch_array($result)) {
  	printf ("fact%s [label=\"%s:%s\", shape=box, fillcolor=lightcyan, fontsize=50, href=\"javascript:void(0)\", onclick=\"get_id('\L', '\N')\"];\n",$row[0],$row[1],$row[2]);
  }
}
mysqli_free_result($result);
...
$sql = "select distinct b2.beliefID, CASE 
	    ...
	    inner join questions q on q.agentID = ab.agentID and q.sessionID = ab.sessionID and q.timestep = ab.timestep
			where b.beliefID = ".$ruleID." and ab.sessionID = '".$sessionID."' and ab.timestep = ".$timestep."
					and b2.isRule = 0
					and b2.beliefID IN (select distinct b.beliefID 
									from arguments a 
									inner join beliefs b on b.beliefID = a.beliefID 
									inner join parent_argument pa on pa.argumentID = a.argumentID 
												and pa.sessionID = a.sessionID 
												and pa.timestep = a.timestep 
									where a.sessionID = '".$sessionID."' and a.timestep=".$timestep.")";
$factConclusions = array();
 $result=mysqli_query($link,$sql);
 if ($result) {
  while ($row = mysqli_fetch_array($result)) {
  	$factConclusions[$row[0]] = "Yes";
  }
}
mysqli_free_result($result);
											
//Rule premise nodes that aren't conclusions
$sql = "select distinct b2.beliefID, CASE 
        WHEN b.isNegated=1 THEN concat('NOT(',p.name,'(',c.name,'))') 
        ELSE concat(p.name,'(',c.name,')') END predicate, ab.level
		from belief_has_premises b
 ...
 if ($result) {
  while ($row = mysqli_fetch_array($result)) {
  	printf ("inference%s [label=\"%s:%s\", shape=box, fillcolor=lightcyan, fontsize=50, href=\"javascript:void(0)\", onclick=\"get_id('\L', '\N')\"];\n",$row[0],$row[1],$row[2]);
	$infToExclude[$row[0]]="Yes";	
  }
}
mysqli_free_result($result);

//Fact nodes that are conclusions
/*
...
*/
 ...
 if ($result) {
  while ($row = mysqli_fetch_array($result)) {
  	printf ("agent%s [label=%s, href=\"javascript:void(0)\", fillcolor=grey ,onclick=\"get_id('\L', '\N')\"];\n",$row[0],$row[1]);
  }
}
mysqli_free_result($result);

//Fact nodes that aren't conclusions
/*
